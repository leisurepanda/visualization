...
void calculateHistogram(unsigned char* data, int w, int h, int d) {
    ...
    // 2. 遍歷每一個 Grid Cell (與 Marching Cubes 遍歷方式相同)
    for (int z = 0; z < d - 1; z++) {
        for (int y = 0; y < h - 1; y++) {
            for (int x = 0; x < w - 1; x++) {
                ...
                // 3. 預估三角形數量
                // 為了簡化，我們遍歷這 8 個點涵蓋的 Iso 範圍
                // 只有當 isoValue 介於 [minVal, maxVal] 之間，這個 Cell 才會有面
                ...
                for (int iso = start; iso <= end && iso < 256; iso++) {
                    ...
                    // 查表：這個 index 會產生多少個三角形？
                    // 我們可以看 kCubeTriangles 裡面有多少組非 -1 的值
                    int triInCell = 0;
...
void drawHistogram(const std::vector<int>& counts, int currentIso) {
    ...
    for (int i = 0; i < 256; i++) {
        float x = (i / 255.0f) * 2.0f - 1.0f; // 映射到 -1 ~ 1
